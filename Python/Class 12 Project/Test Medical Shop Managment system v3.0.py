...
cursor.execute('CREATE TABLE IF NOT EXISTS Bill(Bill_No INT(3), Patient_Name VARCHAR(25), Date DATE)')
'''
...
while True:
    ...
    if ch == 2 :
        querya1 = 'SELECT * FROM Bill'
        cursor.execute(querya1)
        resulta1 = cursor.fetchall()
        ...
        print('------------------------Medical Shop------------------------')
        print("Bill No. ", Bill_No + 1)
        pname = input("Patient\'s Name : ")
        rdoc = input("Reffered by Doctor : ") 
        # initiating writer
        fwriter = csv.writer()
