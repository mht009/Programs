            ...
            if ch1 == 1 :  # Adding Medicine to stock
                ...
                while ans.lower() == 'y':
                    ...
                    if cursor.rowcount == 0 : # Checking for duplicate entry
                        ...
                        print('%0s'%'Code', '%15s'%'Name of Medicine', '%18s'%'Name of Man.', '%10s'%'Batch No.', '%10s'%'Man. Date', '%10s'%'Exp.Date', '%10s'%'Quantity', '%10s'%'Price')
                        for row in result:
                            print('%0s'%row[0], '%15s'%row[1], '%20s'%row[2], '%10s'%row[3], '%10s'%row[4], '%10s'%row[5], '%10s'%row[6], '%10s'%row[7])
